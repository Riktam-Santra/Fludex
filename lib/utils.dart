...
class FludexUtils {
  ...
  Future<Settings> getSettings() async {
    var file = await File('data/settings.json').create(recursive: true);
    var contents = await file.readAsString();
    if (contents == '') {
      file.writeAsString(
        jsonEncode(
  ...
  void saveSettings(bool lightMode, bool dataSaver) async {
    ...
    var encodedJson = jsonEncode(user);
    var file = File('data/settings.json');
  ...
  Future<List<String>?> getAllFilePaths(String globalToken,
    ...
    if (chapter != null) {
      ...
      var filenames =
          await _jsearch.getChapterFilenames(await chapterId, isDataSaverMode);
      var urls = <String>[];
      var chapterData =
          await _jsearch.getChapterDataByChapterId(await chapterId);
